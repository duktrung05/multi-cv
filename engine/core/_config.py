...
class BaseConfig(object):
    ...
    def scaler(self) -> GradScaler:
        if self._scaler is None and self.use_amp and torch.cuda.is_available():
            # Prefer new torch.amp.GradScaler('cuda') API (torch>=2.0),
            # but keep compatibility with older torch.cuda.amp.GradScaler().
            try:
    ...
    def writer(self) -> SummaryWriter:
        if self._writer is None:
            # Put each run into a distinct folder so TensorBoard runs are easy to identify.
            task = getattr(self, 'task', None) or 'task'
            safe_task = str(task).replace('/', '_').replace(' ', '_')
            run_name = f'{safe_task}_{datetime.now().strftime("%Y%m%d_%H%M%S")}'

            if self.summary_dir:
                log_dir = Path(self.summary_dir) / run_name
                self._writer = SummaryWriter(log_dir)
            elif self.output_dir:
                log_dir = Path(self.output_dir) / 'summary' / run_name
                self._writer = SummaryWriter(log_dir)
        return self._writer
