...
class YAMLConfig(BaseConfig):
    ...
    def get_optim_params(cfg: dict, model: nn.Module):
        ...
        # -----------------------------
        # Ultralytics-style auto grouping for MuSGD:
        # - use Muon updates for params with ndim >= 2 (conv/linear weights)
        # - keep bias + norm/logit_scale in SGD-only groups (no decay)
        # This mirrors the logic in Ultralytics BaseTrainer.build_optimizer().
        # -----------------------------
        if str(cfg.get("type", "")).lower() == "musgd" and str(cfg.get("auto_grouping", "")).lower() in {
            "ultralytics",
            "ultralytics_musgd",
        }:
            ...
            for name, p in model.named_parameters():
                if not p.requires_grad:
                    continue
                lname = name.lower()
                is_bias = lname.endswith(".bias") or ".bias" in lname
                is_norm = ("norm" in lname) or (".bn" in lname) or ("batchnorm" in lname)
                is_logit_scale = "logit_scale" in lname

                if p.ndim >= 2 and not is_bias and not is_norm and not is_logit_scale:
                    muon_params.append(p)
                elif is_bias:
                    bias_params.append(p)
                elif is_norm or is_logit_scale:
                    bn_params.append(p)
                else:
                    weight_params.append(p)

            param_groups = [
                # SGD weight decay group
                {"params": weight_params, "lr": lr, "momentum": momentum, "nesterov": nesterov, "weight_decay": decay},
                # SGD no-decay group (norm/logit_scale)
                {"params": bn_params, "lr": lr, "momentum": momentum, "nesterov": nesterov, "weight_decay": 0.0},
                # SGD no-decay group (bias)
                {"params": bias_params, "lr": lr, "momentum": momentum, "nesterov": nesterov, "weight_decay": 0.0},
                # Muon+SGD hybrid group
                {
                    "params": muon_params,
                    "lr": lr,
                    "momentum": momentum,
            ...
            # Drop empty groups to keep optimizer clean
            param_groups = [g for g in param_groups if len(list(g.get("params") or [])) > 0]
            return param_groups
