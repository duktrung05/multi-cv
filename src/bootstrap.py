...
from src.infrastructure.ml import ModelFactory, DGM4ClassifierAdapter
...
def build_container(settings: Settings) -> AppContainer:
    ...
    inspector = ModelFactory.build(
        config=settings.classification_config,
        checkpoint=settings.classification_ckpt,
        device=settings.model_device,
    )
    classifier_adapter = DGM4ClassifierAdapter(inspector)
    classifier_service = ClassificationService(
        classifier_adapter,
