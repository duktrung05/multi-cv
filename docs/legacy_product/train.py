...
def initialize_training_context(args):
    """Load config, distributed runtime, device, and derived ``num_classes`` (bootstrap shared with ``test/test.py``).
    ...
    train_dataset_cfg = (((cfg.yaml_cfg or {}).get('train_dataloader') or {}).get('dataset') or {})
...
def main(args, ) -> None:
    ...
    if yaml.safe_load(config_path.read_text(encoding="utf-8")).get("task_domain") == "dgm4_binary":
        from engine.solver.dgm4_solver import run
        run(str(config_path), args.update)
        return
    ...
    if str(cfg.yaml_cfg.get("task") or "") == "classification":
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, default='configs/dgm4_binary.yml')
