...
def create(type_or_name, global_cfg=GLOBAL_CONFIG, **kwargs):
    ...
    # Filter out unknown kwargs to make config merging robust.
    # (YAML merge can leave stale keys when `type` is overridden.)
    try:
        ...
        if not has_var_kw:
            ...
            module_kwargs = {k: v for k, v in module_kwargs.items() if k in accepted}
    except Exception:
        # If signature introspection fails for any reason, fall back to passing through.
        pass
