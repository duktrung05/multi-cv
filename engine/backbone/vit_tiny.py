...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        norm_layer=None, act_layer=None, use_rope=True, use_abs_pos_embed=False,
    ):
        super().__init__()
        ...
        if use_abs_pos_embed:
            # timm's "no_embed_class" layout (used by DeiT III): pos_embed covers
            # patch tokens only, added before the cls token is concatenated in -
            # the cls token itself gets no positional embedding.
            num_patches = self._model.patch_embed.num_patches
            self._model.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
        else:
            self._model.pos_embed = None
        ...
        if use_rope:
            ...
            self._model.rope_embed = None
        self.init_weights()

    def init_weights(self):
        trunc_normal_(self._model.cls_token, std=.02)
        if self._model.pos_embed is not None:
            trunc_normal_(self._model.pos_embed, std=.02)
        if self._model.rope_embed is not None:
            self._model.rope_embed._init_weights()
        self.apply(self._init_vit_weights)

    def interpolate_pos_encoding(self, H_patches, W_patches):
        """Bicubic-resize the learned grid of patch position embeddings to
        (H_patches, W_patches). No-op when the grid already matches (the common
        case: same res as pretraining). No cls-token slot to preserve - see the
        "no_embed_class" note in __init__.
        """
        ...
        assert orig_size * orig_size == num_patches, (
            "interpolate_pos_encoding requires a square pretrained grid"
        )
        dim = pos_embed.shape[-1]
        patch_pos = pos_embed.reshape(1, orig_size, orig_size, dim).permute(0, 3, 1, 2)
        patch_pos = F.interpolate(patch_pos, size=(H_patches, W_patches), mode="bicubic", align_corners=False)
        patch_pos = patch_pos.permute(0, 2, 3, 1).reshape(1, H_patches * W_patches, dim)
        return patch_pos
    ...
    def forward(self, x):
        ...
        if self._model.pos_embed is not None:
            x_embed = x_embed + self.interpolate_pos_encoding(patch_grid_h, patch_grid_w)
