...
class BaseSolver(object):
    ...
    def _save_config(self):
        """Dump the resolved yaml config next to this run's checkpoints/curves for reproducibility."""
        if not dist_utils.is_main_process():
            return
        yaml_cfg = getattr(self.cfg, 'yaml_cfg', None)
        if not yaml_cfg:
            return
        run_dir = Path(self.writer.log_dir) if self.writer is not None else self.output_dir
        run_dir.mkdir(parents=True, exist_ok=True)
        try:
            import yaml
            with open(run_dir / 'config.yml', 'w', encoding='utf-8') as f:
                yaml.safe_dump(yaml_cfg, f, sort_keys=False, allow_unicode=True)
        except Exception as e:
            print(f'[Config][WARN] failed to write config.yml to {run_dir}: {e}')
