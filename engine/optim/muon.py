...
def muon_update(grad: torch.Tensor, momentum: torch.Tensor, beta: float = 0.95, nesterov: bool = True) -> torch.Tensor:
    ...
    # Muon orthogonalization is defined on 2D matrices.
    # - For conv filters and other >=3D tensors: flatten to (out_dim, -1)
    # - For 1D tensors: skip orthogonalization (momentum/SGD-style update only)
    if update.ndim < 2:
        return update

    orig_shape = update.shape
    if update.ndim > 2:
        update2d = update.view(orig_shape[0], -1)
    else:
        update2d = update

    update2d = zeropower_via_newtonschulz5(update2d)

    # Scale update based on the last two dims when available (matches original intent).
    if grad.ndim >= 2:
        update2d *= max(1.0, float(grad.size(-2)) / float(grad.size(-1))) ** 0.5

    return update2d.view(orig_shape) if update.ndim > 2 else update2d
